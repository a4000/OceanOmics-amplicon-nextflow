...
excl <- c(grep('unknown', basename(list.dirs(paste0(wd, '/01-demultiplexed/', assay), recursive = FALSE))),
          grep('unnamed', basename(list.dirs(paste0(wd, '/01-demultiplexed/', assay), recursive = FALSE))))

sites  <- basename(list.dirs(paste0(wd, '/01-demultiplexed/', assay), recursive = FALSE))[-excl]

path   <- paste0(wd, "/01-demultiplexed/", assay)

#=========================================================================================================
# GENERATE FIXED ERROR RATE
#=========================================================================================================

tags <- read_csv(paste0(wd, "/00-raw-data/indices/",voyage,"_indices.csv"))
...
for(i in sample(1:length(fnFs), 3, replace=FALSE)){
  ...
  ggsave(plot = qualityprofile_Fs, 
         filename = paste0(wd, "/03-dada2/QC_plots/", voyage, "_qualityprofile_Fs_", i, "_", assay,"_raw.png"), 
         height = 5, width = 7)
  
  qualityprofile_Rs <- plotQualityProfile(fnRs[i])
  
  ggsave(plot = qualityprofile_Rs, 
         filename = paste0(wd, "/03-dada2/QC_plots/", voyage, "_qualityprofile_Rs_", i, "_", assay, "_raw.png"), 
         height = 5, width = 7)
}
...
filtered_path    <- file.path(paste0(wd, "/03-dada2/filtered_", voyage, "_", assay))
...
for(i in sample(1:length(fnFs), 3, replace=FALSE)){
  ...
  ggsave(plot = qualityprofile_Fs, 
         filename = paste0(wd, "/03-dada2/QC_plots/", voyage, "_qualityprofile_Fs_", i, "_", assay,"_trimmed.png"), 
         height = 5, width = 7)
  
  qualityprofile_Rs <- plotQualityProfile(filtRs[i])
  
  ggsave(plot = qualityprofile_Rs, 
         filename = paste0(wd, "/03-dada2/QC_plots/", voyage, "_qualityprofile_Rs_", i, "_", assay, "_trimmed.png"), 
         height = 5, width = 7)
}
...
save(errors_forward, errors_reverse, file = paste0(wd, "/03-dada2/errorModel/", voyage, "_", assay, "_fixed_error_rates.RData"))

# visualize the estimated error rates
errorsplot_Fs <- plotErrors(errors_forward, nominalQ = TRUE)
ggsave(plot = errorsplot_Fs, 
       filename = paste0(wd, "/03-dada2/QC_plots/", voyage, "_errorsplot_Fs_", assay,".png"), 
       height = 5, 
       width = 7)
errorsplot_Rs <- plotErrors(errors_reverse, nominalQ = TRUE)
ggsave(plot = qualityprofile_Rs, 
       filename = paste0(wd, "/03-dada2/QC_plots/", voyage, "_errorsplot_Rs_", assay, ".png"), 
       height = 5, 
       width = 7)
...
dada2_analysis1 <- function(voyage = voyage, 
  ...
  # define path
  path         <- paste0(wd, "/01-demultiplexed/", assay, "/", site)
  list.files(path)
  ...
  for(i in sample(1:length(fnFs), 3, replace=FALSE)){
    ...
    ggsave(plot = qualityprofile_Fs, 
           filename = paste0(wd, "/03-dada2/QC_plots/", voyage, "_qualityprofile_Fs_", i, "_", assay,"_", site,"_raw.png"), 
           height = 5, width = 7)
    
    qualityprofile_Rs <- plotQualityProfile(fnRs[i])
    
    ggsave(plot = qualityprofile_Rs, 
           filename = paste0(wd, "/03-dada2/QC_plots/", voyage, "_qualityprofile_Rs_", i, "_", assay,"_", site, "_raw.png"), 
           height = 5, width = 7)
  }
  ...
  # Assigns file names and place filtered files in filtered/sub directory
  filtered_path    <- file.path(paste0(wd, "/03-dada2/filtered_", voyage, "_", assay, "/", site))
  filtFs <- file.path(filtered_path,
                      paste0(sample.names_Fs, "_", assay, "_1_trimmed.fq.gz"))
  filtRs <- file.path(filtered_path,
                      paste0(sample.names_Rs, "_", assay, "_2_trimmed.fq.gz"))
  ...
  saveRDS(out, paste0(wd, "/03-dada2/tmpfiles/", voyage, "_", assay,"_", site, "_filterAndTrim_out.rds"))
  ...
  for(i in sample(1:length(fnFs), 3, replace=FALSE)){
    ...
    ggsave(plot = qualityprofile_Fs, 
           filename = paste0(wd, "/03-dada2/QC_plots/", voyage, "_qualityprofile_Fs_", i, "_", assay,"_", site,"_trimmed.png"), 
           height = 5, width = 7)
    
    qualityprofile_Rs <- plotQualityProfile(filtRs[i])
    
    ggsave(plot = qualityprofile_Rs, 
           filename = paste0(wd, "/03-dada2/QC_plots/", voyage, "_qualityprofile_Rs_", i, "_", assay,"_", site, "_trimmed.png"), 
           height = 5, width = 7)
  }
  ...
  load(paste0(wd, "/03-dada2/errorModel/", voyage, "_", assay, "_fixed_error_rates.RData"))
  #......................................................................................
  ...
  #......................................................................................
  # CHECKPOINT Save the result
  save(derep_forward, derep_reverse, file = paste0(wd, "/03-dada2/tmpfiles/", voyage, "_", assay,"_", site, "_dereplicated.RData"))
  ...
  #......................................................................................
  # CHECKPOINT Save the result
  save(dada_forward, dada_reverse, file = paste0(wd, "/03-dada2/tmpfiles/", voyage, "_", assay, "_", site, "_core_sample_inference.RData"))
  ...
  #......................................................................................
  # CHECKPOINT Save the result
  save(mergers, file = paste0(wd, "/03-dada2/tmpfiles/", voyage, "_", assay, "_", site,"_merged.RData"))
  ...
  #......................................................................................
  # CHECKPOINT Save the result
  saveRDS(seq_table, file = paste0(wd, "/03-dada2/tmpfiles/", voyage, "_", assay, "_", site,"_seq_tab.rds"))
...
for(site in sites){
  ...
  st.all[[paste0(c(voyage, assay, site), collapse = '_')]] <- readRDS(paste0(wd, "/03-dada2/tmpfiles/", voyage, "_", assay, "_", site,"_seq_tab.rds"))
...
out_merge <- function(voyage = voyage, 
  ...
  out_all <- readRDS(paste0(wd, "/03-dada2/tmpfiles/", voyage, "_", assay,"_",site,"_filterAndTrim_out.rds"))
...
for(site in sites){
  ...
  out_all[[paste0(c(voyage, assay, site), collapse='_')]] <- out_merge(voyage, assay, site, wd)
}
...
dada_merge <- function(voyage = voyage, 
  ...
  load(paste0(wd, "/03-dada2/tmpfiles/", voyage, "_", assay, "_", site, "_core_sample_inference.RData"))
...
for(site in sites){
  ...
  dada_forward[[length(dada_forward) + 1]] <- dada_merge(voyage, assay, site, wd)
  dada_reverse[[length(dada_reverse) + 1]] <- dada_merge(voyage, assay, site, wd)
}
...
# Save outputs
saveRDS(merged_seqtab, file = paste0(wd, "/03-dada2/tmpfiles/",voyage, "_", assay, "_seq_tab.rds"))
saveRDS(out_merged, paste0(wd, "/03-dada2/tmpfiles/", voyage, "_", assay,"_filterAndTrim_out.rds"))
save(dada_forward, dada_reverse, file = paste0(wd, "/03-dada2/tmpfiles/", voyage, "_", assay, "_core_sample_inference.RData"))

#......................................................................................
# Finish the dada2 analysis with the merged sequence tables
merged_seq_table <- readRDS(paste0(wd, "/03-dada2/tmpfiles/",voyage, "_", assay, "_seq_tab.rds"))
...
ggsave(plot = seq_hist,
       filename = paste0(wd, "/03-dada2/QC_plots/", voyage, "_ASV_seq_distribution_",assay,".png"),
       height = 10,
       width = 12)
...
# Save plot
ggsave(plot = seq_hist,
       filename = paste0(wd, "/03-dada2/QC_plots/", voyage, "_ASV_seq_distribution_noChim_",assay,".png"),
       height = 10,
       width = 12)

#......................................................................................
# CHECKPOINT Save the result
save(seq_table_nochim, file = paste0(wd, "/03-dada2/tmpfiles/", voyage, "_", assay, "_seq_table_nochim.RData"))
...
out <- readRDS(paste0(wd, "/03-dada2/tmpfiles/", voyage, "_", assay,"_filterAndTrim_out.rds"))
load(paste0(wd, "/03-dada2/tmpfiles/", voyage, "_", assay, "_core_sample_inference.RData"))
...
for(site in sites){
  ...
  sample_names[[paste0(c(voyage, assay, site), collapse='_')]] <- get_samples_names(voyage, assay, site, wd)
}
...
head(track_Fs)
tail(track_Fs)
write_tsv(track_Fs, file = paste0(wd, "/03-dada2/QC_plots/Track_reads_Fw_",voyage, "_", assay, ".tsv"))
...
head(track_Rs)
tail(track_Rs)
write_tsv(track_Rs, file = paste0(wd, "/03-dada2/QC_plots/Track_reads_Rs_",voyage, "_", assay, ".tsv"))
...
ggsave(plot = track_boxplot_Fw, 
       filename = paste0(wd, "/03-dada2/QC_plots/", voyage, "_samples_through_stages_Fw_",assay,".png"), 
       height = 10, 
       width = 12)
ggsave(plot = track_boxplot_Rv, 
       filename = paste0(wd, "/03-dada2/QC_plots/", voyage, "_samples_through_stages_Rv_",assay,".png"), 
       height = 10, 
       width = 12)
...
write_csv(asv_final_table, paste0(wd, "/03-dada2/", voyage, "_" ,assay,"_asv_table.csv")) ## input for phyloseq

# making and writing out a fasta of our final ASV seqs:
asv_fasta <- c(rbind(asv_headers, asv_seqs))
write(asv_fasta, paste0(wd, "/03-dada2/", voyage, "_",assay,".fa")) ## input for blastn
...
write_delim(asv_for_lca, paste0(wd, "/03-dada2/",voyage, "_final_table_",assay,".tsv"), delim = '\t')

lca_input <- asv_for_lca %>%
  rename('#ID' = ASV) %>%
  select(-ASV_sequence)
write_tsv(lca_input, paste0(wd, "/03-dada2/", voyage, "_", assay, "_lca_input.tsv"))
